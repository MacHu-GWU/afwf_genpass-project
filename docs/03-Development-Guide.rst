...
------------------------------------------------------------------------------
在项目的根目录下有一个 `afwf_genpass <../afwf_genpass>`_ 目录. 这个目录就是你的 Alfred Workflow 的源码了. 其中 `workflow.py <../afwf_genpass/workflow.py>`_ 模块定义了 Workflow 的对象实例. 并且将所有需要用到的 ``Handler`` 都注册了. 而 `handlers <../afwf_genpass/handlers>`_ 子模块则包含了所有的 handlers 的实现. 我建议查看所有示例 handlers 的源码来了解如何编写常见的 handler 逻辑.

- `error.py <../afwf_genpass/handlers/error.py>`_: 故意抛出异常, 用于测试异常处理逻辑.
- `memorize_cache.py <../afwf_genpass/handlers/memorize_cache.py>`_: 一个带磁盘缓存的记忆函数.
- `open_file.py <../afwf_genpass/handlers/open_file.py>`_: 可以用来选择并打开文件.
- `open_url.py <../afwf_genpass/handlers/open_url.py>`_: 可以用来选择并在浏览器中打开网页.
- `read_file.py <../afwf_genpass/handlers/read_file.py>`_: 对文件的内容进行读取.
- `write_file.py <../afwf_genpass/handlers/write_file.py>`_: 对文件的内容进行写入.
- `set_settings.py <../afwf_genpass/handlers/set_settings.py>`_: 对数据库进行写入.
- `view_settings.py <../afwf_genpass/handlers/view_settings.py>`_: 对数据库进行读取.
