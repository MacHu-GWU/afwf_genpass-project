...
Example
------------------------------------------------------------------------------
这里我们拿 `memorize_cache.py <../afwf_genpass/handlers/memorize_cache.py>`_ 为例. 它的核心逻辑是根据 Key 生成一个随机数, 并且缓存 5 秒. 返回的对象是一个 ``ScriptFilter`` 对象, 而这个生成的随机数则保存在 ``ScriptFilter.items`` 列表中.
